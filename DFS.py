...
class PuzzleApp(ctk.CTk):
	def __init__(self):
		...
		self.solve_button = ctk.CTkButton(controls, text="Solve DFS", command=self.solve)
	...
	def solve(self):
		...
		self.anim_index = 0
		self.status_label.configure(text=f"Steps: {len(path) - 1} | Max depth: {MAX_DEPTH}")
